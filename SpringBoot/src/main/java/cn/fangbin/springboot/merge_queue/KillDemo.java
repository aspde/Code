...
public class KillDemo {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        System.out.println("------- 库存 ------");
        System.out.println("库存最终数量 :" + killDemo.stock);
    }
    ...
    private void rollback(UserRequest userRequest) {
        if(operateChangeLogList.stream().anyMatch(operateChangeLog -> operateChangeLog.getOrderId().equals(userRequest.getOrderId()))) {
            // 回滚
            boolean hasRollback = operateChangeLogList.stream().anyMatch(operateChangeLog -> operateChangeLog.getOrderId()
                    .equals(userRequest.getOrderId()) && operateChangeLog.getOperateType().equals(2));
